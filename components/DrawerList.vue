    ...
    <article>
      <template v-if="data.name === 'Guided Tour'">
        <div class="description">
          ...
          Guided Tourは、庭園をひとめぐりしながらINAの基本的な機能を把握することのできる自動モードです。
        </div>
        <div class="bigBtn guidedTour" @click="startTour('Guided Tour')">
          Start Tour
        </div>
      </template>
      <template v-else-if="data.name === 'Ramble Tour'">
        <h3>Tour with Annotations</h3>
        <div class="description">
          <!-- eslint-disable-next-line -->
          Ramble Tourは、全てのアノテーションをランダムにめぐってゆく自動モードです。Tour with Annotationsでは、各Annotationの説明を表示しながら進んでゆきます。
        </div>
        <div
          class="bigBtn rambleTour"
          @click="startTour('Ramble Tour with Annotations')"
        >
          Start Tour
        </div>
        <h3>Tour without Annotations</h3>
        <div class="description">
          <!-- eslint-disable-next-line -->
          Ramble Tourは、全てのアノテーションをランダムにめぐってゆく自動モードです。Tour without Annotationsでは、各Annotationの説明を表示せず、園内を移動しつづけます。同時にSoundsを再生させることもできます。
        </div>
        <div
          class="bigBtn rambleTour"
          @click="startRambleTourWithoutAnnotations()"
        >
          Start Tour
        </div>
      </template>
      ...
      <template v-if="data.name !== 'Ramble Tour'">
        <div v-if="data.name === 'Oral Archives'" class="groups">
          <div v-for="(val, key) in groups" :key="key" class="group">
            <div class="head">
              <h5>{{ key }}</h5>
              <div
                class="thumb"
